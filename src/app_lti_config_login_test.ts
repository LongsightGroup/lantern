...
Deno.test('GET /lti/canvas/config.json publishes the pilot Canvas config document', async () => {
  ...
  try {
    ...
    const body = (await response.json()) as {
      ...
      scopes: string[];
      extensions: Array<{ settings: { placements: Array<{ placement: string }> } }>;
    };

    assertEquals(typeof body.oidc_initiation_url, 'string');
    assertEquals(body.scopes, [...CANVAS_LTI_SCOPES]);
    assertEquals(body.extensions[0]?.settings.placements[0]?.placement, 'course_navigation');
    assertEquals(
      body.extensions[0]?.settings.placements.some(
        (placement) => placement.placement === 'resource_selection',
      ),
      true,
    );
  } finally {
...
Deno.test('GET /lti/login accepts Moodle-style login initiation and redirects to the saved authentication endpoint', async () => {
  ...
  assertEquals(redirected.origin + redirected.pathname, 'https://moodle.example/mod/lti/auth.php');
...
Deno.test('GET /lti/login tolerates one extra percent-encoding layer on opaque LMS hints', async () => {
  ...
  assertEquals(
    interopEvents.some((event) => event.detail.path === 'opaque_login_hint_decode'),
    true,
  );
  assertEquals(
    interopEvents.some((event) => event.detail.path === 'opaque_lti_message_hint_decode'),
    true,
  );
});
...
Deno.test("GET /lti/login falls back to Lantern's singular launch route when a non-Canvas LMS omits target_link_uri", async () => {
  ...
  try {
    ...
    assertEquals(redirected.searchParams.get('redirect_uri'), 'http://localhost:8417/lti/launch');
...
Deno.test('GET /lti/login uses the forwarded public origin when Lantern fills a missing non-Canvas target_link_uri', async () => {
  ...
  try {
    ...
    assertEquals(redirected.searchParams.get('redirect_uri'), 'https://lantern.example/lti/launch');
...
Deno.test('GET /lti/login still rejects an omitted Canvas target_link_uri when more than one Lantern callback route is possible', async () => {
  ...
  assertEquals(response.status, 409);
  assertStringIncludes(body, 'Canvas launches use more than one Lantern callback route');
});
