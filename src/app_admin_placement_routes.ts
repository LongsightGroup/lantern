...
export function registerAdminPlacementRoutes(app: Hono, services: AppServices): void {
  app.get('/admin/placements', (context) => {
    const url = new URL(context.req.url);
    const placementId = normalizeOptionalString(url.searchParams.get('placementId'));

    if (placementId !== null) {
      return context.redirect(`/admin/placements/${encodeURIComponent(placementId)}`, 303);
    }
    ...
    return context.html(
      renderPlacementAuditRequestPage({
        notice: {
          ...
          detail: 'Use /admin/placements/:placementId to inspect one reviewed placement.',
        },
      }),
      400,
    );
  ...
  app.get('/admin/placements/:placementId', async (context) => {
    ...
    try {
      ...
      const timeline = await loadPlacementAuditTimeline(repository, snapshot.placement);
