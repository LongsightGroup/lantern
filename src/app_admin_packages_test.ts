...
Deno.test('GET /admin/packages renders the shipped reference apps when no versions exist', async () => {
  ...
  assertStringIncludes(body, 'Open Chapter 4 Asteroids');
  assertStringIncludes(body, 'Open Office Hours Web Lab');
  assertStringIncludes(body, 'Open Quick Study');
  assertStringIncludes(body, 'Chapter 4 Asteroids');
  assertStringIncludes(body, 'Office Hours Web Lab');
  assertStringIncludes(body, 'Quick Study');
  assertStringIncludes(body, 'Import a reference app.');
...
Deno.test('POST /admin/packages/import-reference imports the selected reference app and redirects to the version details page', async () => {
  const repository = createInMemoryPackageReviewRepository();
  const app = createApp({
    getRepository: () => repository,
...
Deno.test('POST /admin/packages/import-reference reopens the existing reference app version when the exact version is already present', async () => {
  const repository = createInMemoryPackageReviewRepository({
...
Deno.test('POST /admin/packages/import-reference imports quick-study when requested', async () => {
  ...
  assertEquals(response.status, 303);
  assertEquals(response.headers.get('location'), '/admin/packages/quick-study/versions/0.1.0');
