...
Deno.test('POST /admin/packages/import-demo imports the demo package and redirects to the dossier', async () => {
  ...
  const app = createApp({
    getRepository: () => repository,
    importDemoPackage: () => Promise.resolve(buildImportedPackageVersion({ version: '0.1.0' })),
  });

  const response = await app.request('http://localhost/admin/packages/import-demo', {
    method: 'POST',
...
Deno.test('POST /admin/packages/:appId/deployment/pin stores the exact approved version id', async () => {
  ...
  assertEquals(response.status, 303);
  assertEquals(response.headers.get('location'), '/admin/packages/chapter-4-asteroids/deployment');
