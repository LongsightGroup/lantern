...
Deno.test('preview service keeps explicit authoring origin and selected content in the runtime session', async () => {
  ...
  try {
    ...
    assertEquals(launched.runtimeSession.launch.submissionMode, 'anonymous_submission');
  } finally {
    await Deno.remove(snapshotRoot, { recursive: true });
  }
});
...
Deno.test('preview service records admin authoring draft launches with the authoring route detail', async () => {
  const snapshotRoot = await Deno.makeTempDir({ prefix: 'lantern-preview-' });
  ...
  try {
    ...
    await Deno.writeTextFile(
      ...
      JSON.stringify({
        ...
        attempt_id: 'attempt-preview-777',
        local_state: null,
      }),
    );
    ...
    const approvedPackage = buildPackageVersionRecord({
      ...
      artifact: {
        ...
        manifestPath: `${snapshotRoot}/manifest.json`,
        entrypointPath: `${snapshotRoot}/dist/index.html`,
        digest: 'sha256:preview-admin-authoring-origin',
      },
      manifestJson: {
        app_id: 'template-app',
        version: '0.1.0',
    ...
    const launched = await launchPreviewRuntimeSession({
      ...
      now: () => new Date('2026-04-08T17:10:00Z'),
      createOpaqueToken: () => crypto.randomUUID(),
    });
    const evidence = await repository.listPreviewEvidence(launched.previewSession.sessionId);

    assertEquals(launched.previewSession.origin, 'adminAuthoringDraft');
    assertEquals(evidence[0]?.summary, "Started an authoring preview in Lantern's runtime.");
    assertEquals(
