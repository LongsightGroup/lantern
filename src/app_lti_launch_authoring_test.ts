...
Deno.test('POST /lti/launch rejects Sakai authoring-mode requests and never creates a runtime session', async () => {
  ...
  await withFetchStub(
    ...
    async () => {
      ...
      const savedState = await repository.getLoginStateByState('state-sakai-authoring');
      const runtimeSession = await repository.getLatestRuntimeSessionByDeploymentId(17);
      ...
      assertEquals(auditEvents[0]?.detail.messageType, LTI_DEEP_LINKING_REQUEST_MESSAGE_TYPE);
      assertEquals(
