...
Deno.test("listScaffoldStarters exposes the curated starter set", () => {
  assertEquals(listScaffoldStarters(), [
    ...
    },
    {
      ...
      description:
        "Minimal browser-graded activity starter with reviewed specs.",
    },
  ]);
});
...
Deno.test("scaffoldLocalAppPackage writes a simple activity package that validates and previews", async () => {
  ...
  try {
    ...
    if (!validation.ok || !validation.appPackage) {
      throw new Error(
        `Expected scaffolded package to validate: ${
    ...
    const contentSource = await Deno.readTextFile(
      `${outputRoot}/content/activity.json`,
    );
    const fixturesSource = await Deno.readTextFile(
      `${outputRoot}/preview/fixtures.json`,
    );
    ...
    assertEquals(
      validation.appPackage.fixtureData.launch.activity_id,
      "cell-review",
    );
    assertEquals(
      validation.appPackage.fixtureData.attempt_id,
      "attempt_cell_review_demo",
    );
    assertEquals(
      validation.appPackage.previewTests[0]?.assert.text,
      "Cell Review",
    );
...
Deno.test("scaffoldLocalAppPackage writes a browser autograder package with coherent metadata artifacts", async () => {
  ...
  try {
    ...
    if (!validation.ok || !validation.appPackage) {
      throw new Error(
        `Expected scaffolded browser autograder to validate: ${
          JSON.stringify(validation.issues)
        }`,
      );
    }

    const previewTestsSource = await Deno.readTextFile(
      `${outputRoot}/preview/tests.json`,
    );
    const graderSpecSource = await Deno.readTextFile(
      `${outputRoot}/grading/specs/checks.spec.js`,
    );
    const entrypointSource = await Deno.readTextFile(
      `${outputRoot}/dist/index.html`,
    );
    const appSource = await Deno.readTextFile(`${outputRoot}/dist/app.js`);
    ...
    assertEquals(
      validation.appPackage.manifest.authoring?.kind,
      "browser_autograder",
    );
    assertEquals(
      validation.appPackage.fixtureData.launch.activity_id,
    ...
    const runnerResponse = await harness.handle(
      new Request(
        "http://localhost/_lantern/runtime/browser-grader/runner.js",
        {
          headers: {
            authorization,
          },
