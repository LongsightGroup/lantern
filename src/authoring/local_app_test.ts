...
import {
  ...
  preflightLocalAppPackageSource,
} from "./local_app.ts";
import {
  renderValidationFailure,
  serializeValidationResult,
} from "../../tools/app_validate.ts";
import {
...
Deno.test("preflightLocalAppPackageSource rejects empty preview tests with file-scoped fix guidance", async () => {
  const result = await preflightLocalAppPackageSource(
    buildPackageSource({ previewTests: [] }),
  );
  ...
  assertStringIncludes(
    result.diagnostics[0]?.fix ?? "",
    "Add at least one named preview test",
  );
});
  ...
  const output = renderValidationFailure({
    ...
    diagnostics: [
      ...
      buildDiagnostic({
        ...
        message:
          "Preview tests file must contain at least one named assertion.",
...
Deno.test("serializeValidationResult emits stable JSON for success and failure", async () => {
  ...
  if (!success.ok) {
    throw new Error(
      `Expected valid package source: ${JSON.stringify(success.diagnostics)}`,
    );
  }
  ...
  assertEquals(
    (successJson.app as Record<string, unknown>).previewTestCount,
    1,
  );
  ...
  assertEquals(
    ((failureJson.diagnostics as unknown[])?.[0] as Record<string, unknown>)
      .file,
    "/preview/tests.json",
  );
});
