...
const AUTHORING_REFERENCE_APP_IDS = ["template-app", "web-checkup"] as const;

export async function loadAuthoringReferenceExamples(): Promise<
  AuthoringReferenceExample[]
> {
  return await Promise.all(
    AUTHORING_REFERENCE_APP_IDS.map((appId) =>
      loadAuthoringReferenceExample(appId)
    ),
  );
}
...
async function loadAuthoringReferenceExample(
  ...
  const source = createFileSystemPackageSource(
    getReferencePackageSourceRoot(appId),
  );
...
async function readReferenceManifest(
  ...
  if (
    ...
    throw new Error(
      `Reference example ${appId} manifest.json must be a JSON object.`,
    );
  }
