...
const CANVAS_PRIVACY_LEVEL_EXTENSION =
...
async function loadCanvasOpenIdProviderConfiguration(input: {
  ...
  if (!response.ok) {
    throw new Error(
      `Canvas openid_configuration fetch failed with status ${response.status}.`,
    );
  }
...
async function submitCanvasRegistration(input: {
  ...
  const response = await input.fetch(input.registrationEndpoint, {
    ...
    body: JSON.stringify(
      buildCanvasRegistrationRequest(input.appOrigin, input.appTitle),
    ),
  });

  if (!response.ok) {
    throw new Error(
      `Canvas registration failed with status ${response.status}.`,
    );
  }
