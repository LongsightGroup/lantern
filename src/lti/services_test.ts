...
Deno.test('LTI services client requests RS256 client-credentials tokens and publishes final AGS scores with server-owned identity', async () => {
    ...
    await withFetchStub(
      (input, init) => {
        ...
        if (url === 'https://sso.canvaslms.com/login/oauth2/token') {
          ...
          assertStringIncludes(body, `client_id=${binding.clientId}`);
      ...
      async () => {
        ...
        assertEquals(token.accessToken, 'canvas-access-token');
