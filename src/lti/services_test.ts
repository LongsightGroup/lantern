...
Deno.test('LTI services client reuses a launch lineitem or creates one from the lineitems container before score publish', async () => {
  ...
  const result = await withFetchStub(
    (input, init) => {
      ...
      assertEquals((init?.headers as HeadersInit | undefined) !== undefined, true);
  ...
  assertEquals(result.lineItemUrl, 'https://canvas.example/api/lti/courses/42/line_items/9');
    ...
    await withFetchStub(
      (input, init) => {
        ...
        if (matchingBinding !== undefined) {
          ...
          assertStringIncludes(body, `deployment_id=${matchingBinding.deploymentId}`);
...
Deno.test('LTI services client tolerates sparse line-item payloads and creates a fresh record instead of aborting grading flow', async () => {
  ...
  assertEquals(requests, [`GET ${ags.lineitemsUrl}`, `POST ${ags.lineitemsUrl}`]);
  assertEquals(result.created, true);
  assertEquals(result.lineItemUrl, 'https://canvas.example/api/lti/courses/42/line_items/11');
});
...
Deno.test('LTI services client retries one Canvas score publish with a fresh token after a 401', async () => {
  ...
  assertEquals(tokens, ['Bearer canvas-access-token', 'Bearer canvas-access-token-refreshed']);
  assertEquals(publication.status, 202);
});
