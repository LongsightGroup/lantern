...
Deno.test("deep linking response builder signs one LtiDeepLinkingResponse JWT and echoes the saved data claim", async () => {
  ...
  assertEquals(typeof verified.payload.jti, "string");
  assertEquals(verified.payload.nonce, verified.payload.jti);
  assertEquals(
...
Deno.test("deep linking response builder returns one launch content item with Lantern's singular launch URL and placement key", async () => {
  ...
  assertEquals(
    (contentItem.presentation as Record<string, unknown>).documentTarget,
    "iframe",
  );
  assertEquals(
...
Deno.test("deep linking response builder falls back to the first accepted presentation target when iframe is unavailable", async () => {
  const response = await buildDeepLinkingResponseSubmission({
    session: buildDeepLinkingSessionRecord({
      acceptPresentationDocumentTargets: ["window", "embed"],
    }),
    deployment: buildDeploymentRecord({
      binding: buildDeploymentBinding(),
    }),
  ...
  assertEquals(
    (contentItems[0]?.presentation as Record<string, unknown>).documentTarget,
    "window",
  );
});
