...
Deno.test(
  "deep linking response builder signs one LtiDeepLinkingResponse JWT and echoes the saved data claim",
  async () => {
    ...
    assertEquals(
      verified.payload["https://purl.imsglobal.org/spec/lti/claim/version"],
      "1.3.0",
    );
    assertEquals(
...
Deno.test(
  "deep linking response builder returns one launch content item with Lantern's singular launch URL and placement key",
  async () => {
    ...
    assertEquals(
      (contentItem.custom as Record<string, unknown>)[
        LANTERN_PLACEMENT_CUSTOM_KEY
      ],
      "placement-456",
    );
    assertEquals(
      typeof (contentItem.custom as Record<string, unknown>)[
        LANTERN_PLACEMENT_CUSTOM_KEY
      ],
      "string",
    );
...
Deno.test(
  "deep linking response builder derives optional line-item metadata from reviewed package facts",
  async () => {
    const placement = buildReviewedPlacementRecord({
    ...
    assertEquals(
      lineItem.resourceId,
      buildReviewedPlacementLineItemResourceId(placement),
    );
  },
);
