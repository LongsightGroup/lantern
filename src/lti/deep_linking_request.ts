...
export async function validateDeepLinkingRequest(input: {
  ...
  const state = requireTrimmedValue(input.state, 'Deep Linking state is required.');
  const idToken = requireTrimmedValue(input.idToken, 'Deep Linking id_token is required.');
  ...
  try {
    ...
    throw new Error('Deep Linking id_token signature or issuer validation failed.');
  }
  ...
  const nonce = requireStringClaim(payload.nonce, 'Deep Linking nonce is required.');
  ...
  if (deploymentId !== loginState.deploymentId) {
    throw new Error('Deep Linking deployment_id did not match the saved login state.');
  }

  if (targetLinkUri !== loginState.targetLinkUri) {
    throw new Error('Deep Linking target_link_uri did not match the saved login state.');
  }
...
function requireDeepLinkingRouteTarget(targetLinkUri: string): void {
  ...
  try {
    ...
    throw new Error('Deep Linking target_link_uri must be an absolute Lantern URL.');
  }

  if (url.pathname !== DEEP_LINKING_ROUTE_PATH || url.search !== '') {
    throw new Error(`Unsupported Deep Linking target_link_uri ${targetLinkUri}.`);
  }
}
...
function parseDeepLinkingSettingsClaim(value: unknown): DeepLinkingSettings & {
  ...
  const settings = requireRecordClaim(value, 'Deep Linking settings claim is required.');
...
function parseAcceptTypes(value: unknown): DeepLinkingAcceptType[] {
  ...
  if (unsupported.length > 0) {
    throw new Error(`Unsupported Deep Linking accept_types: ${unsupported.join(', ')}.`);
  }
