...
export function validateLtiAudience(input: {
  ...
  const audiences = Array.isArray(input.aud)
    ? input.aud
    : input.aud
    ? [input.aud]
    : [];
  ...
  if (!audiences.includes(input.clientId)) {
    throw new Error(
      `${input.subject} audience did not include client_id ${input.clientId}.`,
    );
  }
  ...
  if (authorizedParty !== null && authorizedParty !== input.clientId) {
    throw new Error(
      `${input.subject} azp did not match client_id ${input.clientId}.`,
    );
  }

  if (audiences.length > 1 && authorizedParty === null) {
    throw new Error(
      `${input.subject} azp is required when aud has multiple values.`,
    );
  }
}
...
export function requireRecordClaim(
  value: unknown,
  message: string,
...
export function optionalTypedStringClaim(
  value: unknown,
  message: string,
): string | null {
  if (value === undefined || value === null) {
