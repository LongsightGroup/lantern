...
import {
  type AuthoringGeneratedDraftView,
  renderAuthoringPage,
...
export function registerAdminAuthoringRoutes(
  ...
  app.get(
    "/admin/packages/:appId/versions/:version/authoring",
    async (context) => {
  ...
  app.post(
    "/admin/packages/:appId/versions/:version/authoring/generate",
    async (context) => {
      const appId = context.req.param("appId");
      const version = context.req.param("version");
  ...
  app.post(
    "/admin/packages/:appId/versions/:version/authoring/save",
    async (context) => {
      const appId = context.req.param("appId");
      const version = context.req.param("version");
      let state: AuthoringRouteState | null = null;
      ...
      try {
        ...
        const generatedFiles = parseGeneratedFiles(
          formData,
          state.draft.authoringPaths,
        );
...
function normalizeGeneratedFiles(
  ...
  for (const file of files) {
    ...
    if (!allowedPaths.has(path)) {
      throw new Error(
        `${messagePrefix} ${path} is outside the approved authoring file set.`,
      );
    }
  ...
  return [...normalizedFiles.values()].sort((left, right) =>
    left.path.localeCompare(right.path)
  );
}

function readRepeatedFormStrings(
