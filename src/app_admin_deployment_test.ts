          ...
          packageVersions: [
            buildPackageVersionRecord({
              ...
              accessibilityReview: buildAccessibilityReview({
                contrast: 'fail',
                failureNotes: 'Contrast fixes still pending for the score meter.',
...
Deno.test('POST /admin/packages/:appId/deployment/lti-profile saves an override for only the selected deployment slot', async () => {
  ...
  assertEquals(
    (await repository.getDeploymentBySlug('chapter-4-asteroids-moodle'))?.ltiProfileOverride,
    'certification',
  );
  assertEquals(
    (await repository.getDeploymentBySlug('chapter-4-asteroids-pilot'))?.ltiProfileOverride,
    null,
  );
  assertEquals(
