...
Deno.test('repository records one-way approval and rejection decisions with optional notes', async () => {
  await withRepositoryTestDatabase(async ({ repository }) => {
    ...
    const persistedApproved = await repository.getPackageVersionById(approvalCandidate.id);
    const persistedRejected = await repository.getPackageVersionById(rejectionCandidate.id);
...
Deno.test('repository persists one exact signed runtime contract per approved version and keeps earlier approvals immutable', async () => {
  await withRepositoryTestDatabase(async ({ repository }) => {
    ...
    assertEquals(approvedV010.runtimeContract.artifactDigest, approvedV010.artifact.digest);
    assertEquals(approvedV010.runtimeContract.entrypoint, approvedV010.entrypoint);
    assertEquals(approvedV010.runtimeContract.capabilities, approvedV010.capabilities);
    ...
    assertEquals(refetchedV010.runtimeContractSignature, approvedV010.runtimeContractSignature);
    assertEquals(approvedV020.runtimeContract.packageVersion, approvedV020.version);
    assertEquals(
      approvedV020.runtimeContractSignature === approvedV010.runtimeContractSignature,
      false,
    );
  });
});
