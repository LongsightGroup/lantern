  ...
  return {
    async createPreviewSession(record) {
      return await withClient(pool, async (client) => {
        return await withTransaction(client, 'create_preview_session', async (transaction) => {
          ...
          try {
            const insertResult = await transaction.queryObject<PreviewSessionRow>({
              text: `
      ...
      return await withClient(pool, async (client) => {
        const result = await client.queryObject<PreviewSessionRow>({
          text: `
            ...
            LIMIT 1
          `,
          args: origin === undefined ? [packageVersionId] : [packageVersionId, origin],
          camelCase: true,
        });
    ...
    async appendPreviewEvidence(input) {
      return await withClient(pool, async (client) => {
        return await withTransaction(client, 'append_preview_evidence', async (transaction) => {
          const sessionResult = await transaction.queryObject<{ sessionId: string }>({
            text: `
                SELECT session_id
          ...
          const insertResult = await transaction.queryObject<PreviewEvidenceRow>({
            text: `
