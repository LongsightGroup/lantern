...
Deno.test("repository seeds authoring drafts from approved browser autograder package versions", async () => {
  await withRepositoryTestDatabase(async ({ repository }) => {
    const approvedRecord = await registerApprovedBrowserAutograderVersion(
      repository,
    );
    ...
    assertEquals(
      created.baseSnapshotRoot,
      approvedRecord.artifact.snapshotRoot,
    );
...
async function registerApprovedBrowserAutograderVersion(
  repository: Pick<
    ...
    "registerPackageVersion" | "approvePackageVersion"
  >,
) {
