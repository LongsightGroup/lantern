  ...
  return {
    async saveCanvasRegistration(input) {
      return await withClient(pool, async (client) => {
        return await withTransaction(client, 'save_canvas_registration', async (transaction) => {
          ...
          const conflictingPendingRegistration = await transaction.queryObject<DeploymentRow>({
            text: `
                  ${DEPLOYMENT_SELECT}
