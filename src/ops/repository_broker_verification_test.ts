...
Deno.test("ops repository keeps broker verification scoped to the exact deployment and supported path", async () => {
  await withPackageReviewTestDatabase(async (pool) => {
    ...
    await repository.recordBrokerVerificationRun(
      {
        ...
        certificationState: null,
        summary:
          "1EdTech does not list Lantern for the Moodle verification path.",
    ...
    assertEquals(
      rowsByLms.get("canvas")?.brokerVerification?.supportedPath,
    ...
    assertEquals(
      rowsByLms.get("moodle")?.brokerVerification?.internal?.source,
      "ci",
    );
    assertEquals(
      rowsByLms.get("moodle")?.brokerVerification?.official.state,
      "notCertified",
    );
    assertEquals(
      rowsByLms.get("sakai")?.brokerVerification?.supportedPath,
      SAKAI_SUPPORTED_SCOPE,
    );
    assertEquals(
      rowsByLms.get("sakai")?.brokerVerification?.internal?.status,
      "pending",
    );

    assertExists(moodleDetail);
    assertEquals(
      moodleDetail.brokerVerification?.supportedPath,
      MOODLE_SUPPORTED_SCOPE,
    );
    assertEquals(moodleDetail.brokerVerification?.internal?.source, "ci");
    assertEquals(
      moodleDetail.brokerVerification?.official.state,
      "notCertified",
    );
    assertEquals(
    ...
    assertEquals(
      sakaiDetail.brokerVerification?.supportedPath,
      SAKAI_SUPPORTED_SCOPE,
    );
    assertEquals(sakaiDetail.brokerVerification?.internal?.status, "pending");
    assertEquals(
      sakaiDetail.brokerVerification?.official.state,
      "notCertified",
    );
  });
});
