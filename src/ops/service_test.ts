...
Deno.test('ops service retries failed grade publication against the attempt-scoped runtime session and updates the existing ledger row', async () => {
  ...
  assertEquals(
    interopEvents.some((event) => event.detail.ltiProfileId === 'governedCompatibility'),
    true,
  );
});
  ...
  try {
    const result = await withFetchStub(
      (input, init) => {
        ...
        if (
          url === 'https://canvas.example/api/lti/courses/42/line_items/9/scores' &&
          method === 'POST'
        ) {
...
Deno.test('runtime publication fails on Canvas 401 without retry when certification disables the saved service retry path', async () => {
  ...
  try {
    ...
    assertEquals(result.publishError?.message, 'Canvas score publish failed with status 401.');
...
Deno.test('ops service fails a manual retry Canvas 401 without retry when certification disables the saved service retry path', async () => {
  ...
  const result = await opsServiceModule.retryFailedGradePublication({
    ...
    publishScore: (input: {
      ...
      return Promise.reject(new Error('Canvas score publish failed with status 401.'));
    },
  });
  const savedPublication = await repository.getGradePublicationByAttemptId('attempt-123');
  const interopEvents = await repository.listAuditEventsByEventType('interop.path_used');
