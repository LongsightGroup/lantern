...
function buildDeepLinkingOperatorSummary(item: ControlPlaneDiagnosticItem): string {
  const savedDeployment = describeSavedDeploymentPath(item.detail);
...
function buildRuntimeOperatorSummary(item: ControlPlaneDiagnosticItem): string {
  ...
  const code = readStringDetail(item.detail, 'code');
  const deliverySubstrate = readStringDetail(item.detail, 'deliverySubstrate');
  ...
  switch (item.eventType) {
    ...
    case 'runtime.session.integrity_failed':
      if (code === 'runtime_delivery_failed' && deliverySubstrate === 'dynamic_worker') {
        return 'Dynamic Worker delivery failed before Lantern could serve the reviewed runtime bytes.';
      }

      if (code === 'runtime_file_missing') {
        return 'A reviewed runtime file was missing from the approved package snapshot.';
      }
