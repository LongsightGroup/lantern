...
export function isCertificationWorkflowKey(value: string): value is CertificationWorkflowKey {
  return value === 'core' || value === 'deepLinking' || value === 'nrps' || value === 'ags';
}
...
export function readNumberDetail(detail: Record<string, unknown>, key: string): number | null {
  const value = detail[key];
...
export function readLtiProfileIdDetail(detail: Record<string, unknown>): LtiProfileId | null {
...
export function readLtiProfileSourceDetail(
  ...
  return value === 'lanternDefault' || value === 'deploymentOverride' ? value : null;
}
