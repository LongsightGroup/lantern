...
import { insertAttemptEvidenceArtifact, insertAuditEvent } from './repository_test_core_support.ts';
...
Deno.test('ops repository returns deployment detail snapshots with recent launches, the latest checks, and only failed diagnostics', async () => {
  await withSeededOpsRepositoryTest(async (pool, repository) => {
    ...
    assertEquals(detail.recentLaunches[0]?.ltiProfileId, 'governedCompatibility');
    assertEquals(detail.recentLaunches[0]?.ltiProfileSource, 'lanternDefault');
    assertEquals(detail.latestLaunch?.detail.ltiProfileId, 'governedCompatibility');
    assertEquals(detail.latestNrpsRead?.status, 'succeeded');
    assertEquals(detail.latestNrpsRead?.detail.ltiProfileSource, 'lanternDefault');
...
Deno.test('ops repository returns the latest deployment-scoped AGS smoke verification result for the viewed deployment', async () => {
  await withSeededOpsRepositoryTest(async (pool, repository) => {
    ...
    assertEquals(moodleDetail.latestAgsSmoke?.detail.ltiProfileId, 'certification');
    assertEquals(moodleDetail.latestAgsSmoke?.detail.ltiProfileSource, 'deploymentOverride');
    ...
    assertEquals(sakaiDetail.latestAgsSmoke?.detail.ltiProfileSource, 'lanternDefault');
  });
});
...
Deno.test('ops repository exposes runtime session and sandbox boundary evidence for deployment detail snapshots', async () => {
  await withSeededOpsRepositoryTest(async (pool, repository) => {
    ...
    assertEquals(runtimeDetail.latestRuntimeSession?.sandboxModel, 'contained_browser_runtime');
    assertEquals(runtimeDetail.latestRuntimeSession?.boundary, 'app_runtime_origin');
    assertEquals(runtimeDetail.latestRuntimeSession?.deliverySubstrate, 'dynamic_worker');
    assertEquals(
    ...
    assertEquals(runtimeDetail.latestRuntimeOutcome?.eventType, 'runtime.session.exited');
    ...
    assertEquals(runtimeDetail.latestRuntimeOutcome?.sandboxModel, 'contained_browser_runtime');
    assertEquals(runtimeDetail.latestRuntimeOutcome?.boundary, 'app_runtime_origin');
    assertEquals(runtimeDetail.latestRuntimeOutcome?.deliverySubstrate, 'dynamic_worker');
    assertEquals(
    ...
    assertEquals(runtimeDetail.latestAnonymousEvidence?.[0]?.artifactId, 'artifact-001');
    assertEquals(runtimeDetail.latestAnonymousEvidence?.[0]?.contentType, 'application/json');
    assertEquals(runtimeDetail.latestAnonymousEvidence?.[0]?.byteSize, 128);
    assertEquals(runtimeDetail.latestAnonymousEvidence?.[0]?.sha256, 'sha256:artifact-001');
    assertEquals(runtimeDetail.latestAnonymousEvidence?.[0]?.createdAt, '2026-03-24T12:36:30.000Z');
    assertEquals(
