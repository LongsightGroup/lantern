...
Deno.test('ops repository surfaces latest deployment binding evidence per deployment in inventory and detail snapshots', async () => {
  await withSeededOpsRepositoryTest(async (pool, repository) => {
    ...
    const rows = await repository.listControlPlaneDeployments();
    const rowsByLms = new Map(rows.map((row) => [row.binding?.lms ?? 'missing', row] as const));
    const canvasInstallEvidence = readInventoryInstallEvidence(rowsByLms.get('canvas'));
    const moodleInstallEvidence = readInventoryInstallEvidence(rowsByLms.get('moodle'));
    const sakaiInstallEvidence = readInventoryInstallEvidence(rowsByLms.get('sakai'));
    ...
    assertEquals(canvasInstallEvidence.summary, 'Saved Canvas deployment binding.');
    ...
    assertEquals(moodleInstallEvidence.summary, 'Saved Moodle deployment binding.');
    assertEquals(moodleInstallEvidence.detail.lms, 'moodle');
    assertEquals(rowsByLms.get('moodle')?.updatedAt === moodleInstallEvidence.occurredAt, false);

    assertExists(sakaiInstallEvidence);
    assertEquals(sakaiInstallEvidence.occurredAt, '2026-03-24T12:31:00.000Z');
    assertEquals(sakaiInstallEvidence.summary, 'Saved Sakai deployment binding.');
