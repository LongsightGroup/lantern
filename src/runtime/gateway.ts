...
export async function submitEvidenceArtifact(input: {
  ...
  await input.repository.recordAuditEvent({
    ...
    status: 'accepted',
    summary: 'Stored an anonymous evidence artifact through the runtime gateway.',
    detail: {
...
function validateBrowserGraderResult(input: {
  ...
  if (input.browserGraderResult === null) {
    denyRuntimeBroker({
      ...
      message: 'Browser grading requires browserGraderResult in finalizeAttempt().',
      capability: 'finalize_attempt',
      detail: {},
    });
  }
  ...
  if (input.browserGraderResult.scoreMaximum !== reviewedConfig.scoreMaximum) {
    denyRuntimeBroker({
      ...
      message: 'Browser grader scoreMaximum must match the reviewed grading max score.',
      capability: 'finalize_attempt',
      detail: {
  ...
  for (const specResult of input.browserGraderResult.specResults) {
    if (!reviewedSources.has(specResult.source)) {
      denyRuntimeBroker({
        ...
        message: 'Browser grader specResults must stay inside reviewed grader spec files.',
        capability: 'finalize_attempt',
        detail: {
    ...
    if (seenSources.has(specResult.source)) {
      denyRuntimeBroker({
        ...
        message: 'Browser grader specResults cannot repeat the same reviewed spec file.',
        capability: 'finalize_attempt',
        detail: {
          source: specResult.source,
        },
...
async function loadFinalizeEvidenceArtifacts(input: {
  ...
  session: RuntimeSessionRecord;
  attemptId: string;
}): Promise<Array<Pick<AttemptEvidenceArtifactRecord, 'artifactId' | 'kind' | 'fileName'>>> {
  if (input.session.launch.submissionMode !== 'anonymous_submission') {
    return [];
  }

  const evidenceArtifacts = await input.repository.listAttemptEvidenceArtifacts(input.attemptId);
...
function sanitizeEvidenceArtifactFileName(fileName: string): string {
  ...
  const sanitized = baseName.replaceAll(/[^A-Za-z0-9._-]+/g, '-').replaceAll(/^-+|-+$/g, '');
...
async function createEvidenceArtifactDigest(bytes: Uint8Array): Promise<string> {
