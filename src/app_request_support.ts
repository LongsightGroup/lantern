...
export function parseLanternDefaultLtiProfileForm(formData: FormData): LtiProfileId {
  return requireLtiProfileId(
    requireTrimmedFormValue(formData.get('defaultLtiProfile'), 'Choose one supported LTI profile.'),
  );
}

export function parseDeploymentLtiProfileOverrideForm(formData: FormData): LtiProfileId | null {
...
export async function readLoginRequest(context: Context): Promise<ReadLoginRequestResult> {
  if (context.req.method === 'GET') {
    ...
    const ltiMessageHint = normalizeOpaqueHint(url.searchParams.get('lti_message_hint'));

    return {
      request: {
        iss: requireTrimmedString(url.searchParams.get('iss'), 'LTI issuer is required.'),
        loginHint: loginHint.value,
        targetLinkUri: normalizeNullableString(url.searchParams.get('target_link_uri')),
  ...
  const ltiMessageHint = normalizeOpaqueFormValue(formData.get('lti_message_hint'));
...
function normalizeOpaqueFormValue(value: FormDataEntryValue | null) {
  return normalizeOpaqueHint(formValueAsString(value));
}

function requireOpaqueFormValue(value: FormDataEntryValue | null, message: string) {
  return requireOpaqueHint(formValueAsString(value), message);
}

function requireOpaqueHint(
  value: string | null,
  message: string,
): {
  value: string;
