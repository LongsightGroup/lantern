...
Deno.test(
  "renderPlacementAuditPage shows selected content, reviewed package identity, Canvas context, and placement status",
  () => {
...
Deno.test(
  "renderPlacementAuditPage includes deep-linking, preview, and reviewer timeline entries when evidence exists",
  () => {
    const html = renderPlacementAuditPage({
      snapshot: {
        placement: buildReviewedPlacementRecord({
