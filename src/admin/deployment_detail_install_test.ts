...
Deno.test('POST /admin/packages/:appId/deployment/install saves one exact Canvas binding', async () => {
  ...
  try {
    ...
    }).request('http://localhost/admin/packages/chapter-4-asteroids/deployment/install', {
      method: 'POST',
      headers: {
...
Deno.test('POST /admin/packages/:appId/deployment/install requires the exact Moodle binding fields', async () => {
  ...
  assertEquals(response.status, 409);
  assertStringIncludes(await response.text(), 'Moodle Authentication request URL is required.');
...
Deno.test('POST /admin/packages/:appId/deployment/install requires the exact Sakai binding fields', async () => {
  ...
  assertEquals(response.status, 409);
  assertStringIncludes(await response.text(), 'Sakai OIDC authentication URL is required.');
  assertEquals(await repository.getDeploymentBySlug('chapter-4-asteroids-sakai'), null);
});
