  ...
  return `<article class="step-card stack">
      ...
      <div class="facts">
        <div class="fact">
          ...
          <span class="fact-value">${escapeHtml(input.slot.deployment.slug)}</span>
        </div>
        <div class="fact">
      ...
      <div class="callout">
        ...
        <div class="fact">
          ...
          <code class="inline-code">${escapeHtml(
            input.canvasConfigUrl ?? 'APP_ORIGIN is required before Lantern can publish the config URL.',
          )}</code>
        </div>
      </div>
      ${
      ...
      <div class="facts">
        <div class="fact">
          ...
          <span class="fact-value">${escapeHtml(
            nrpsVerification === null ? 'Not run yet' : formatDateTime(nrpsVerification.checkedAt),
          )}</span>
        </div>
        <div class="fact">
        ...
        <div class="fact">
          ...
          <span class="fact-value">${escapeHtml(
            nrpsVerification?.memberCount === null || nrpsVerification?.memberCount === undefined
              ? 'Not recorded'
...
function renderVersionPinForm(
  ...
  return `<div class="stack">
      ...
      <form method="post" action="/admin/packages/${escapeHtml(appId)}/deployment/pin" class="stack">
                    ...
                    .map(
                      (version) =>
                        `<option value="${escapeHtml(String(version.id))}" ${
                          slot.deployment.enabledPackageVersionId === version.id ? 'selected' : ''
                        }>Version ${escapeHtml(version.version)} · ${escapeHtml(version.title)}</option>`,
                    )
                    .join('')
            }
          </select>
    ...
    case 'canvas': {
      ...
      return `
        ...
        <div class="fact">
          ...
          <span class="fact-value">${escapeHtml(describeBindingValue(binding?.issuer))}</span>
        </div>
        <div class="fact">
          <span class="fact-label">Canvas Client ID</span>
          <span class="fact-value">${escapeHtml(describeBindingValue(binding?.clientId))}</span>
        </div>
        <div class="fact">
...
function describeBindingStatusHeading(
  ...
  switch (slot.lms) {
    case 'canvas':
      ...
      return slot.deployment.enabledPackageVersionId !== null && canvasConfigUrl !== null
...
function getCanvasBinding(binding: DeploymentBinding | null): CanvasDeploymentBinding | null {
