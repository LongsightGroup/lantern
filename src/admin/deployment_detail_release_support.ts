...
export function describeBindingStatusHeading(
...
export function describeManagedSlotIntro(
  lms: ManagedDeploymentSlot["lms"],
): string {
  switch (lms) {
    case "canvas":
...
export function hasPendingCanvasRegistration(
  slot: ManagedDeploymentSlot,
): boolean {
  return (
    slot.lms === "canvas" && slot.persisted &&
    getCanvasBinding(slot.deployment.binding) === null
  );
}
