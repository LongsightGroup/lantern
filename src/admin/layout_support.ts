...
export function defaultNav(activePath?: string): NavItem[] {
  return [
    ...
    },
    {
      ...
      icon: `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3l7 4v5c0 5-3.5 8-7 9-3.5-1-7-4-7-9V7l7-4z"/><path d="M9 12l2 2 4-4"/></svg>`,
      active: activePath?.startsWith('/admin/verification') ?? false,
      children: buildVerificationNav(activePath),
    },
    {
      label: 'Placements',
      href: '/admin/placements',
      icon: `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M12 21s-6-4.35-6-10a6 6 0 1 1 12 0c0 5.65-6 10-6 10z"/><circle cx="12" cy="11" r="2.5"/></svg>`,
      ...
      <nav class="sidebar-nav">
        ...
        ${nav
          .map(
            ...
            <a class="sidebar-link ${item.active ? 'active' : ''}" href="${escapeHtml(
              item.href,
  ...
  return `<header class="page-header">
      ...
      <p class="page-desc">${escapeHtml(input.intro)}</p>
        ...
        <aside class="operator-chip" aria-label="Current operator">
          <span class="operator-chip-label">${escapeHtml(identity.detail)}</span>
          <strong class="operator-chip-name">${escapeHtml(identity.displayName)}</strong>
        </aside>
      </div>
      ${input.pageNav ? `<div class="page-header-nav">${input.pageNav}</div>` : ''}
    </header>`;
}
...
function resolveAdminIdentity(): AdminIdentity {
  const displayName =
    readOptionalEnvIdentityValue('LANTERN_OPERATOR_NAME') ??
    readOptionalEnvIdentityValue('USER') ??
    readOptionalEnvIdentityValue('LOGNAME') ??
    'Local operator';
...
function readOptionalEnvIdentityValue(name: string): string | null {
  try {
    return normalizeIdentityValue(readEnv(name));
  } catch {
    return null;
  }
}
...
function buildVerificationNav(activePath?: string): NavChildItem[] {
  if (!activePath?.startsWith('/admin/verification')) {
    return [];
  }
  ...
  return [
    {
      label: 'Checklist',
      href: '/admin/verification',
      active: activePath === '/admin/verification',
    },
    {
      label: 'Official evidence',
      href: '/admin/verification/official',
      active: activePath.startsWith('/admin/verification/official'),
    },
    {
      label: 'Add result',
      href: '/admin/verification/new',
      active: activePath.startsWith('/admin/verification/new'),
    },
    {
      label: 'Lantern default',
      href: '/admin/verification/lti-profile',
      active: activePath.startsWith('/admin/verification/lti-profile'),
    },
  ];
}
