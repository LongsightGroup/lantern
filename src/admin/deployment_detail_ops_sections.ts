...
function renderActivityFact(label: string, value: string, summary: string): string {
...
function renderDimensionRow(label: string, dimension: ControlPlaneHealthDimension | null): string {
...
function formatActivityTimestamp(snapshot: DeploymentActivitySnapshot | null | undefined): string {
...
function describeGradePublication(snapshot: DeploymentGradePublicationSnapshot): string {
...
function describeOverallStatus(status: ControlPlaneHealthStatus | null): string {
  switch (status) {
    case 'healthy':
      return 'Healthy';
    case 'attention':
...
function describeDiagnosticKind(kind: ControlPlaneDiagnosticItem['kind']): string {
  switch (kind) {
    case 'launch':
      return 'Launch';
    case 'nrps':
