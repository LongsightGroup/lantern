...
import {
  renderPackagePageNav,
  supportsAuthoringDrafts,
} from "./package_navigation.ts";
...
export function renderPackageOverviewPage(input: {
  ...
  const latestApprovedVersion =
    input.history.find((version) => version.approvalStatus === "approved") ??
      null;
  const connectedDeployments = input.deployments.filter((deployment) =>
  ...
  return renderAdminLayout({
    ...
    body: `<section class="panel">
      <div class="panel-body stack">
        <div class="panel-header">
          <div class="stack">
            ...
            <h2>${
      escapeHtml(input.appTitle)
    } <span class="app-inline-meta">App ID ${
      escapeHtml(input.appId)
    }</span></h2>
            <p>${
      escapeHtml(
        latestVersion?.description ?? "No app description was provided.",
      )
    }</p>
          </div>
      ...
      latestApprovedVersion === null
        ...
        : `<a class="button-ghost" href="/admin/packages/${
          ...
          escapeHtml(latestApprovedVersion.version)
        }/preview">Test launch</a>`
    }
          </div>
        </div>
        <div class="facts">
      ...
      escapeHtml(latestVersion?.version ?? "None yet")
    }</span>
          </div>
          <div class="fact">
      ...
      escapeHtml(
        latestApprovedVersion?.version ?? "No approved version",
          ...
          <div class="fact">
            ...
            <span class="fact-value">${
      escapeHtml(String(connectedDeployments.length))
    }</span>
          </div>
          <div class="fact">
            <span class="fact-label">Updated</span>
            <span class="fact-value">${
      escapeHtml(formatDateTime(latestVersion?.importedAt ?? null))
    }</span>
          </div>
        </div>
      </div>
      ...
      <div class="panel-body stack">
        <div class="panel-header">
          <div class="stack">
            ...
            <p>Latest approved stays the current reviewed baseline. LMS setup decides which approved version is live.</p>
          </div>
        </div>
    ...
    <section class="panel">
      ...
      input.deployments.length === 0
        ...
        : `<div class="table-list">
              ${
          input.deployments.map((deployment) => renderDeploymentRow(deployment))
            .join("")
        }
            </div>`
    }
...
function renderOverviewVersionRow(input: {
  ...
  const { version, isLatestVersion, isLatestApproved, liveDeploymentCount } =
    input;
  const accessibility = summarizeAccessibilityReview(version);
  const summaryChip = isLatestApproved
    ? '<span class="chip version-summary-chip">Latest approved</span>'
    : isLatestVersion
    ? '<span class="chip version-summary-chip">Latest upload</span>'
    : "";

  return `<article class="table-row version-row${
    isLatestApproved ? " version-row-current" : ""
  }">
    ...
    escapeHtml(
      approvalStatusLabel(version.approvalStatus),
        ...
        <p class="line-copy">${
    escapeHtml(
      version.reviewNotes ?? version.description ??
    ...
    <div class="table-row-meta">
      <span><strong>Live rollout</strong> ${
    escapeHtml(formatLiveRolloutLabel(liveDeploymentCount))
  }</span>
...
function buildLiveDeploymentCounts(
  deployments: DeploymentRecord[],
): Map<number, number> {
  ...
  for (const deployment of deployments) {
    if (
      deployment.binding === null || deployment.enabledPackageVersionId === null
    ) {
      continue;
    }
...
function formatLiveRolloutLabel(liveDeploymentCount: number): string {
  if (liveDeploymentCount === 0) {
    return "Not live";
  }

  return `Live in ${liveDeploymentCount} LMS setup${
    liveDeploymentCount === 1 ? "" : "s"
  }`;
}
...
function renderDeploymentRow(deployment: DeploymentRecord): string {
  return `<article class="table-row">
    <div class="table-row-top">
      <div class="stack">
        <p class="line-title">${
    escapeHtml(formatLmsLabel(deployment.lmsType))
  }</p>
        <p class="line-copy">${
    escapeHtml(
      deployment.binding === null
    ...
    escapeHtml(formatDateTime(deployment.updatedAt))
  }</span>
