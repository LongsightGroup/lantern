...
export function renderPlacementAuditSection(): string {
  return `<section class="panel">
  ...
  return `<article class="table-row">
    <div class="table-row-top">
      ...
      <div class="button-row">
        <a class="button-ghost" href="${escapeHtml(dossierHref)}">Open dossier</a>
    ...
    <div class="table-row-meta">
      ...
      <span><strong>Enabled version</strong> ${escapeHtml(
        deployment.enabledPackageVersion ?? 'Not pinned yet',
      )}</span>
      <span><strong>Approval state</strong> ${escapeHtml(
        deployment.approvalStatus === null
          ? 'Not reviewed'
          : approvalStatusLabel(deployment.approvalStatus),
      )}</span>
      <span><strong>Enablement state</strong> ${escapeHtml(
        describeEnablementState(deployment),
      )}</span>
      <span><strong>Current health</strong> ${escapeHtml(
        describeHealthLabel(deployment.health.overallStatus),
      )}</span>
      <span><strong>Latest launch</strong> ${escapeHtml(
        describeActivitySnapshot(deployment.lastLaunchStatus, deployment.lastLaunchAt),
      )}</span>
      <span><strong>Latest AGS write</strong> ${escapeHtml(
        describeGradePublicationSnapshot(
          deployment.lastGradePublishStatus,
          deployment.lastGradePublishAt,
        ),
      )}</span>
      <span><strong>Latest NRPS read</strong> ${escapeHtml(
        describeActivitySnapshot(deployment.lastNrpsReadStatus, deployment.lastNrpsReadAt),
      )}</span>
