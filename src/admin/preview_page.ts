...
function renderPreviewEvidenceDetail(appId: string, record: PreviewEvidenceRecord): string {
  ...
  const fileName = readPreviewEvidenceFileName(record.detail) ?? 'Evidence artifact';
...
function resolvePreviewEvidenceArtifactUrl(
  ...
  return artifactId === null ? null : `/admin/packages/${appId}/deployment/evidence/${artifactId}`;
}

function readPreviewEvidenceFileName(detail: Record<string, unknown>): string | null {
...
function isPreviewScreenshotEvidence(detail: Record<string, unknown>): boolean {
  return (
    readPreviewEvidenceString(detail, 'kind') === 'screenshot_png' ||
    readPreviewEvidenceString(detail, 'contentType') === 'image/png'
  );
}

function readPreviewEvidenceString(detail: Record<string, unknown>, key: string): string | null {
