...
export function renderPlacementAuditPage(input: {
  ...
  return renderAdminLayout({
    ...
    body: `<section class="panel">
      ...
      escapeHtml(placement.contentTitle ?? placement.contentPath)
      ...
      escapeHtml(String(input.snapshot.previewEvidenceCount))
...
export function renderPlacementAuditRequestPage(input: {
  ...
  return renderAdminLayout({
    ...
    heading: "Placement audit",
    intro:
      "Load one reviewed placement id to inspect selected content, reviewed package identity, and durable reviewer evidence.",
    activePath: "/admin/packages",
    breadcrumbs: [
      { label: "Packages", href: "/admin/packages" },
      { label: "Placements" },
    ...
    body: `<section class="panel">
      <div class="panel-body stack">
        <p class="section-label">Open a placement</p>
        <h2>Placement id is required.</h2>
        <p>Use the canonical route <code>/admin/placements/:placementId</code> to inspect one reviewed placement.</p>
      </div>
    </section>`,
  });
}
...
function renderTimelineRows(
  ...
  const rows = [...timeline].sort((left, right) => {
  ...
  if (snapshot.latestPreviewSessionId !== null) {
    rows.unshift(`<article class="line-item">
      ...
      <p class="line-copy">Preview session ${
      escapeHtml(snapshot.latestPreviewSessionId)
...
function describePlacementStatus(
  status: PlacementAuditSnapshot["status"],
