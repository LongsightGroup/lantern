...
Deno.test(
  "deep linking picker separates save from Canvas return and only enables return from one saved selection",
  () => {
    ...
    assertStringIncludes(
      readyHtml,
      "/lti/deep-linking/sessions/deep-linking-session-123/submit",
    );
    assertStringIncludes(
      readyHtml,
    ...
    assertStringIncludes(
      blockedHtml,
      "disabled>Return to Canvas</button>",
    );
  },
);

Deno.test(
  "deep linking picker blocks Canvas return when Lantern cannot verify the session token",
  () => {
