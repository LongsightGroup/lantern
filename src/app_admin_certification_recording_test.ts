...
Deno.test('POST /admin/verification records workflow-keyed internal certification evidence and leaves unrelated workflows untouched', async () => {
  ...
  formData.set('summary', 'Deep Linking certification evidence passed for the Moodle deployment.');
  ...
  assertEquals(statusesByWorkflow.get('deepLinking')?.latestInternal?.deploymentRecordId, 2);
  assertEquals(
    statusesByWorkflow.get('deepLinking')?.latestInternal?.deploymentLabel,
    'Chapter 4 Asteroids Moodle Deployment',
  );
  assertEquals(statusesByWorkflow.get('deepLinking')?.latestInternal?.status, 'passed');
  assertEquals(
...
Deno.test('POST /admin/verification records official certification evidence globally and keeps internal workflow evidence unchanged', async () => {
  ...
  const internalResponse = await app.request('http://localhost/admin/verification', {
    method: 'POST',
    headers: { Origin: 'http://localhost' },
  ...
  const officialResponse = await app.request('http://localhost/admin/verification', {
    method: 'POST',
    headers: { Origin: 'http://localhost' },
  ...
  assertEquals(officialEvidence.directoryUrl, 'https://example.test/certification/1edtech');
  assertEquals(
