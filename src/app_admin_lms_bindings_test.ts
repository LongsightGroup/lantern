...
Deno.test('POST /admin/packages/:appId/deployment/install saves the Moodle binding without mutating Canvas or Sakai slots', async () => {
  ...
  formData.set('authenticationRequestUrl', 'https://moodle.example/mod/lti/auth.php');
  ...
  const canvas = await repository.getDeploymentBySlug('chapter-4-asteroids-pilot');
  const moodle = await repository.getDeploymentBySlug('chapter-4-asteroids-moodle');
  const sakai = await repository.getDeploymentBySlug('chapter-4-asteroids-sakai');
  ...
  assertEquals(auditEvents[0]?.detail.deploymentSlug, 'chapter-4-asteroids-moodle');
});
...
Deno.test('POST /admin/packages/:appId/deployment/install saves the Sakai binding without mutating Canvas or Moodle slots', async () => {
  ...
  formData.set('oidcAuthenticationUrl', 'https://sakai.example/imsti/sakai_oidc_login');
  formData.set('accessTokenUrl', 'https://sakai.example/imsti/sakai_access_token');
  ...
  assertEquals(auditEvents[0]?.detail.deploymentSlug, 'chapter-4-asteroids-sakai');
});
...
Deno.test('POST /admin/packages/:appId/deployment/pin pins the selected LMS slot and records its identity', async () => {
  ...
  const auditEvents = await repository.listAuditEventsByEventType('deployment.version_pinned');
  assertEquals(auditEvents.length, 1);
  assertEquals(auditEvents[0]?.detail.lms, 'moodle');
  assertEquals(auditEvents[0]?.detail.deploymentSlug, 'chapter-4-asteroids-moodle');
