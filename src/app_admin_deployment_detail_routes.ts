  ...
  app.get(
    ...
    async (context) => {
      try {
        ...
        const artifact = await repository.getAttemptEvidenceArtifactById(
          artifactId,
        );
        ...
        const bytes = await services.evidenceArtifactStore.readBytes(
          artifact.storageKey,
        );
        ...
        return new Response(
          new Blob([responseBody], { type: artifact.contentType }),
          {
            status: 200,
            headers: {
