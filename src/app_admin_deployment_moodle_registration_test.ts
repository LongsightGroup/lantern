...
Deno.test("GET /admin/packages/:appId/deployment/register/moodle completes automatic setup and returns a Moodle close page", async () => {
  ...
  try {
    ...
    const registrationState = await repository.createDynamicRegistrationState({
      state: "moodle-registration-state-123",
          ...
          return new Response(
            JSON.stringify({
              ...
              registration_endpoint:
                "https://moodle.example/mod/lti/openid-registration.php",
            }),
            {
              status: 200,
      ...
      async () => {
        const params = new URLSearchParams({
          ...
          openid_configuration:
            "https://moodle.example/mod/lti/openid-configuration.php",
...
Deno.test("GET /admin/packages/:appId/deployment/register/moodle records the deployment_id fallback path when Moodle omits it from the response body", async () => {
  const previousOrigin = Deno.env.get("APP_ORIGIN");
  Deno.env.set("APP_ORIGIN", "http://localhost:8417");
  ...
  try {
    ...
    const registrationState = await repository.createDynamicRegistrationState({
      state: "moodle-registration-state-fallback",
      appId: "chapter-4-asteroids",
      lms: "moodle",
      createdAt: "2030-03-24T00:00:00Z",
      expiresAt: "2030-03-24T00:30:00Z",
    ...
    assertEquals(
      interopEvents[0]?.detail.path,
      "dynamic_registration_deployment_id_fallback",
    );
    assertEquals(interopEvents[0]?.detail.deploymentIdSource, "openid_query");
  } finally {
    restoreEnv("APP_ORIGIN", previousOrigin);
  }
});
