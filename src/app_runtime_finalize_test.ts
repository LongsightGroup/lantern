...
import { EXAMPLE_SNAPSHOT_ROOT, restoreEnv, withFetchStub } from './app_test_support.ts';
import {
  ...
  try {
    await withFetchStub(
      ...
      async () => {
        ...
        const attemptAuditEvents = await repository.listAuditEventsByEventType('attempt.finalized');
  ...
  try {
    await withFetchStub(
      ...
      async () => {
        ...
        }).request('http://localhost/runtime/sessions/runtime-session-123/finalize', {
          method: 'POST',
          headers: {
            Authorization: 'Bearer runtime-token-123',
            'content-type': 'application/json',
