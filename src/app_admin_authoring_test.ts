...
const AUTHORING_PROMPT =
  "Write a browser_autograder check for missing alt text.";
const GENERATION_NOTE =
  "Added a browser_autograder assertion for alt text coverage.";
...
Deno.test("POST /authoring/generate returns notes and diffs without saving the draft yet", async () => {
  ...
  assertEquals(
    ...
    ["template-app", "web-checkup"],
  );
  assertStringIncludes(
    capturedInput.currentFiles[0]?.contents ?? "",
...
Deno.test("POST /authoring/save persists generated files and AI provenance into the draft record", async () => {
  ...
  assertEquals(draft.files.map((file) => file.relativePath), [
    "/grading/specs/checks.spec.js",
  ]);
  assertStringIncludes(
    draft.files[0]?.contents ?? "",
